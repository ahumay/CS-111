...
///////////////////////// connect to server
	...
	SSL_set_fd(sslID, socketFD);
	SSL_connect(sslID);
///////////////////////// 
	
	char buffer[256];
	sprintf(buffer, "ID=%s\n", UID);
	SSL_write(sslID, buffer, strlen(buffer));
	if (logFlag){
...
void ehandler(char indicator, int errorCode){
	if (errorCode < 0){
		...
		if (indicator == 'o'){
			fprintf(stderr, "\nFailure waiting for OpenSSL.\n");
		}
		if (indicator == 't'){
			fprintf(stderr, "\nFailure creating a TLS session.\n");
		}
		if (indicator == 'c'){
			fprintf(stderr, "\nFailure to connect.\n");
			exit(2);
